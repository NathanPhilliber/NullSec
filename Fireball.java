...
public class Fireball extends Weapon
{
    ...
    private void beam()
    {
        ...
        for (int i=0; i<=5; i++)
        {
            getWorld().addObject(new Beam(beamAngle), (int)Math.round(getX()+i*8*Math.cos(beamAngle*2*Math.PI/360)), (int)Math.round(getY()+i*8*Math.sin(beamAngle*2*Math.PI/360)));
        }
        for (int i=0; i<=5; i++)
        {
